...
class TwitchApp:
    def __init__(self, root):
        ...
        self.root.configure(
            bg="#f0f0f0"
        )  # Arka plan rengini hafif gri yapalım
        ...
        self.channel_label.grid(
            row=0, column=0, columnspan=2, pady=20, sticky="w", padx=20
        )

        # Kullanıcı Listesi (Genişlik daraltıldı)
        self.user_listbox = tk.Listbox(
            root, font=("Arial", 12), width=20, height=20
        )
        self.user_listbox.grid(row=1, column=0, padx=20, pady=10, sticky="n")

        # Kullanıcı Seçimi için Buton
        self.select_user_button = tk.Button(
            root,
            text="Select User",
            command=self.on_user_select,
            font=("Arial", 12),
        )
        self.select_user_button.grid(
            row=2, column=0, padx=20, pady=10, sticky="n"
        )
        ...
        self.chat_text.grid(
            row=1, column=1, rowspan=2, padx=20, pady=10, sticky="nsew"
        )
        ...
        self.connect_button = tk.Button(
            ...
            command=self.connect_to_twitch,
            font=("Arial", 12, "bold"),
            width=14,
            height=2,
            bg="#7F00FF",
        ...
        self.root.grid_columnconfigure(
            1, weight=1
        )  # Sohbet mesajları için genişletilebilir kolon
        self.root.grid_rowconfigure(
            1, weight=1
        )  # Sohbet mesajları için genişletilebilir satır
    ...
    def connect_to_twitch(self):
        try:
            ...
            # messagebox.showinfo(
            #     "Authorization",
            #     "Please authorize the app in the opened browser window.",
            # )
            ...
            messagebox.showinfo(
                "User Info", f"Connected as: {display_name} (ID: {user_id})"
            )
            ...
            self.channel_label.config(
                text=f"Channel: #{channel}", fg="#1db954"
            )  # Kanal adı yeşil renkte gösterilsin
    ...
    def connect_to_twitch_chat(self, username, channel):
        try:
            ...
            self.irc.connect()
            messagebox.showinfo(
                "Success", f"Connected to Twitch Chat on channel #{channel}!"
            )
        except Exception as e:
            messagebox.showerror("Error", str(e))
    ...
    def on_user_select(self):
        ...
        if selection:
            ...
            messagebox.showinfo(
                "User Selected",
                f"Now showing messages only from: {self.selected_user}",
            )
        else:
            self.selected_user = None
